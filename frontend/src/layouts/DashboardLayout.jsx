  ...
  return (
    <div className="min-h-screen bg-white flex">
      ...
      <aside
        ...
        `}
      >
        <Sidebar onNavigate={() => { if (window.innerWidth < 1024) setSidebarOpen(false) }} />
      </aside>

      {/* Main area */}
      <div className={`${sidebarOpen ? 'lg:ml-60' : 'ml-0'} flex-1 flex flex-col min-h-screen transition-all duration-300 ease-out`}>
        {/* Top bar */}
        <header className="h-16 border-b border-[#E5E7EB] bg-white flex items-center px-4 lg:px-8 gap-4 sticky top-0 z-20">
          {/* Hamburger */}
          <button
            className="text-[#6B7280] hover:text-[#374151] transition-colors p-1 -ml-1 flex-shrink-0"
